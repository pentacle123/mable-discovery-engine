...
interface Props {
  ...
  /** When embedded inside CreatorCollabColumn, skip outer box + toggle. */
  embedded?: boolean;
  /** Prioritize MICRO tier (1만~10만) first. Use inside CreatorCollabColumn. */
  microFirst?: boolean;
  /** Override static COLLAB_TEMPLATES with an idea-specific line (e.g. brandMoment). */
  collabTemplate?: string;
}

export default function CreatorMatchPanel({
  queries,
  contentType,
  embedded = false,
  microFirst = false,
  ...
  const staticTemplate =
    (COLLAB_TEMPLATES as Record<string, string>)[contentType] ??
    ((contentType as string) === 'E' ? COLLAB_TEMPLATE_E : COLLAB_TEMPLATES.A);
  const template = collabTemplate || staticTemplate;
    ...
    async function iterate() {
      ...
      for (const q of validQueries) {
        ...
        try {
          ...
          const raw = Array.isArray(data.creators) ? (data.creators as CreatorMatch[]) : [];
          const items = microFirst ? sortMicroFirst(raw) : raw;
          if (cancelled) return;
          if (items.length >= 3) {
  ...
  const content = (
    <>
      {/* Static collab template — hide when embedded (parent column shows it) */}
      {!embedded && (
        <div
          style={{
            fontSize: 12,
...
/**
 * Prioritize MICRO (10K–100K) first, then NANO (1K–10K), then at most 1 MACRO.
 * Used when the panel is embedded inside the creator-collab column, where
 * micro-tier matches are strongly preferred per the creator-collab spec.
 */
function sortMicroFirst(creators: CreatorMatch[]): CreatorMatch[] {
  const micro = creators.filter((c) => c.subscriberCount >= 10_000 && c.subscriberCount <= 100_000);
  const nano = creators.filter((c) => c.subscriberCount >= 1_000 && c.subscriberCount < 10_000);
  const macro = creators.filter((c) => c.subscriberCount > 100_000);
  return [...micro, ...nano, ...macro.slice(0, 1)];
}
